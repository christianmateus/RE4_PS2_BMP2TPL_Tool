...
namespace TPLUtil
{
    ...
    {
        public void TPLtoBMP(string filename)
        {
            ...
            // WRITING DATA 
            // -------------------------------
            BinaryWriter bw = new BinaryWriter(File.Open(Path.GetFileNameWithoutExtension(filename) + ".bmp", FileMode.Create));
            ...
            if (bmp.colors == 16)
            {
                ...
                {
                    bw.Write(SwapNibbles(pixels[pixel]));
                }
            }
            else
            {
        ...
        public void BMPtoTPL(string filename)
        {
            BinaryReader br = new BinaryReader(File.Open(filename, FileMode.Open));
            TPL tpl;
            ...
            // WRITING DATA
            //-------------------------------
            BinaryWriter bw = new BinaryWriter(File.Open(Path.GetFileNameWithoutExtension(filename) + ".tpl", FileMode.Create));
            ...
            if (tpl.bitDepth == 8)
            {
                for (int pixel = 0; pixel < bmpPixels.Length; pixel++)
                {
                    bw.Write(SwapNibbles(bmpPixels[pixel]));
                }
            }
            else
            {
